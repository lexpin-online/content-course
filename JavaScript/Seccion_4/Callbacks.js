...
function iterarArray(array, callback) {

    for (elemento = 0; elemento < array.length; elemento++) {
        callback(`Hola  ${array[elemento]}!`);
    };

};

// Al llamar la funcion "iterarArray()", se le pasan 2 parametros, un array y un callbak,
// el callback viajara y sera iniciado en la funcion "iterarArray()", recibira datos para 
// procesar al retornar, en este caso un string con las palabras "Hola", el elemento que 
// corresponda el ciclo actual, en cada vuelta del for dentro del la funcion "iterarArray()",
...
// "iterarArray()", en este caso es .push con la finalidad de cargar cada string
...
let saludos = [];
iterarArray(personas, function(parametro) {
    saludos.push(parametro);
    }
);
...
/*
    ...
    La función debería iterar sobre el array e invocar el callback para cada elemento.
    la función "encontrar" deberá devolver el primer elemento que terotne true en el callback.
*/
...
let resultado = encontrar(numeros, (elemento)=> {
...
/*** EJEMPLO 5 ***/
// Elabora una funcion permita añadir elementos a un carrito mediante llamado de callbacks recursivo.

let carrito = "🛒";

//Funcion que agrega un producto al carrito tras 2 segundos y ejecuta un callback.
function agregarProducto(callback){
    setTimeout(()=>{
        carrito += "📦";
        callback();
        },2000);
